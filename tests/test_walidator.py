# -*- coding: utf-8 -*-
"""Test walidatora schematu. Dowód, że kontrakt danych łapie 'elegancko opisany nonsens'
i przepuszcza poprawne węzły. Wykrywany przez unittest/pytest (CI)."""
...
class TestWalidator(unittest.TestCase):
    ...
    def test_poprawny_wezel_przechodzi(self):
        wezel = {'id': 'M045', 'typ': 'mechanizm', 'tytul': 'X',
                 'status_epistemiczny': 'zweryfikowane', 'wersja': '0.1'}
        self.assertEqual(self.w.sprawdz_wezel(wezel), [])

    def test_nieznany_typ_lapany(self):
        wezel = {'id': 'M045', 'typ': 'wymyslony_typ', 'tytul': 'X',
                 'status_epistemiczny': 'zweryfikowane', 'wersja': '0.1'}
        self.assertTrue(any('nieznany typ' in n for n in self.w.sprawdz_wezel(wezel)))

    def test_prefiks_niezgodny_z_typem_lapany(self):
        # E-prefiks (dowod) przy typie mechanizm = elegancki nonsens
        wezel = {'id': 'E045', 'typ': 'mechanizm', 'tytul': 'X',
                 'status_epistemiczny': 'zweryfikowane', 'wersja': '0.1'}
    ...
    def test_zly_status_epistemiczny_lapany(self):
        wezel = {'id': 'M045', 'typ': 'mechanizm', 'tytul': 'X',
                 'status_epistemiczny': 'wymyslony', 'wersja': '0.1'}
    ...
    def test_id_niezgodny_z_wzorcem_lapany(self):
        wezel = {'id': 'mechanizm-45', 'typ': 'mechanizm', 'tytul': 'X',
                 'status_epistemiczny': 'zweryfikowane', 'wersja': '0.1'}
    ...
    def test_wiarygodnosc_dowodu_poza_zakresem_lapana(self):
        wezel = {'id': 'E003', 'typ': 'dowod', 'tytul': 'X',
                 'status_epistemiczny': 'zweryfikowane', 'wiarygodnosc': 9}
        self.assertTrue(any("poza zakresem 1-5" in n for n in self.w.sprawdz_wezel(wezel)))

    def test_poprawny_dowod_przechodzi(self):
        wezel = {'id': 'E003', 'typ': 'dowod', 'tytul': 'X',
                 'status_epistemiczny': 'zweryfikowane', 'wiarygodnosc': 4}
        self.assertEqual(self.w.sprawdz_wezel(wezel), [])
